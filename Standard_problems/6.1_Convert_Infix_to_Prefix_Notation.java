...
class Main{
	...
	static String reverse_String(String s){
		...
		for(int i = 0; i<s.length();i++){
			String character = s.charAt(s.length()-1-i)+"";
			if(character.equals("(")){
				rev_s = rev_s + (")");
			}
				...
				rev_s = rev_s + character;
			}
		}
		return rev_s;
	}

	static String convertInfix_prefix(String s){
	...
	//	System.out.println(rev_s);
		...
		String rev_prefix = infix_to_postfix(rev_s);
		
		String prefix = reverse_String(rev_prefix);
	...
	public static void main(String[] args) {
         ...
         System.out.println(convertInfix_prefix(exp));
	}
