...
class Main{
	...
	static String convertPrefix_Postfix(String str){
		...
		for(int i = str.length()-1; i>=0 ; i--){
			...
			convertPrefix_Postfix_util(stack,character);
			//printStack(stack);
	...
	static void convertPrefix_Postfix_util(Stack stack, String character){
		...
		if(isOperator(character)){
			...
			if(!stack.isEmpty()){
				...
				if(!stack.isEmpty()){
					...
					stack.push(operand1 + operand2 + character);
				}
			}
		}
		else{
	...
	public static void main(String[] args) {
		 ...
		 String exp = "*-A/BC-/AKL";
         System.out.println(convertPrefix_Postfix(exp));
	}
