...
class Main{
	...
	static String convertPostfix_Prefix(String str){
		...
		for(int i = 0; i<=str.length()-1 ; i++){
			...
			convertPostfix_Prefix_util(stack,character);
			//printStack(stack);
	...
	static void convertPostfix_Prefix_util(Stack stack, String character){
		...
		if(isOperator(character)){
			...
			if(!stack.isEmpty()){
				...
				if(!stack.isEmpty()){
					...
					stack.push(character  + operand2 +operand1);
				}
			}
		}
		else{
	...
	public static void main(String[] args) {
		 ...
		 String exp = "ABC/-AK/L-*";
         System.out.println(convertPostfix_Prefix(exp));
	}
